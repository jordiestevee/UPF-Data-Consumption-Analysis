...
encoded_consumption_poblenou <- data.frame(
...
for (i in 1:nrow(magnitud_activa_per_UPF_per_hora_Poblenou)) {
  ...
  encoded_consumption_poblenou[i,] <- new_row_data
}

model <- lm(is_high_consumption~es_classe+es_dissabte+es_diumenge+es_festiu+es_no_lectiu+es_vacances+es_avaluacio+es_benvinguda, data = encoded_consumption_poblenou)
